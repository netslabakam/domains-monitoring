...
<input class="aCharts" type="hidden" value="json" />
*/
...
var aCharts = {
    ...
    template: {
        layer: function (o) {
            ...
            switch (o.type){
                case 'circle':
                    temps.push(
                        $('<canvas width="' + o.width * 2 + '" height="' + o.height * 2 + '" style="width:'+o.width+'px;height:'+o.height+'px;"></canvas>')
                    );
                    break;
                default:
                    if(o.title != undefined){
                        temps.push(
                            $('<div />', {
                                class: 'aCharts__title',
                                text: o.title,
                                style: (o.textColor != undefined ? 'color:' + o.textColor + ';' : ''),
                                append: $('<span />', {
                                    text: o.val + '%'
                                })
                            })
    ...
    shape: {
        circle: function (layer, o) {
            ...
            layer.drawText({
                ...
                fontSize: prop.radius * 0.5,
                x: prop.x , y: prop.y,
                fillStyle: o.textColor
            });

            layer.animateLayer('stroke', {
                    start: 0, end: 360 / 100 * o.val
                }, 200, function(layer) {
                    $(this).animateLayer('text', {
                        visible: true
                    }, 'fast', 'swing');
                });
        ...
        line: function (layer, o) {
            var temps = [];

            temps.push(
                $('<div />', {
                    class: 'aCharts_line__percent',
            ...
            if(o.mark != undefined){
                temps.push(
                    $('<div />', {
                        class: 'aCharts_line__mark',
                        style: 'background-color:' + o.color + ';left:' + o.mark + '%;'
                    })
                );
            }

            layer[layer[1] == undefined ? 0 : 1].append(temps);
    ...
    init: function () {
        $('.aCharts').each(function (i, el) {
            var $el = $(el);
            var $wrap = $('<div />', {
               class: 'aCharts_wrap'
            });
            var o = aCharts.JSONparse($el.val());
            var _layer = null;
            ...
            if(o.type == undefined || o.bgColor == undefined || o.color == undefined ){
                console.log('При инициализации произошла ошибка');
                return;
            }

            _layer = aCharts.template.layer(o);
            _layer = aCharts.shape[o.type](_layer, o);
            ...
            if(_layer.length > 1){
                setTimeout(function(){
                    var percent = _layer[1].find('.aCharts_line__percent');
                    percent.css('width', percent.data('width') + '%');
                }, 600)
            }
        });
