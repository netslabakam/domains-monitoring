...
class sfValidatorNumber extends sfValidatorBase
{
  ...
  protected function configure($options = array(), $messages = array())
  {
    ...
    $this->addOption('min');
    $this->addOption('max');
    $this->addOption('phone', false);

    $this->setMessage('invalid', 'Только цифры');
  }
  ...
  {
    if($this->getOption('phone')){
      $value = Page::perfectPhone($value);
    }
    ...
    if ($this->getOption('phone') && Page::perfectPhone($clean) === null)
    {
      throw new sfValidatorError($this, 'Неправильный формат', array('value' => $value));
    }
