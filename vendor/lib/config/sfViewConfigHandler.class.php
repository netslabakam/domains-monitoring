...
EOF;
  ...
  static protected function mergeConfig($config)
  {
    ...
    if(sfConfig::get('sf_min_view'))
    {
      if(!is_dir(sfConfig::get('sf_web_dir') . '/css/min'))
      {
        mkdir(sfConfig::get('sf_web_dir') . '/css/min');
      }
      
      $min_content = '';
      foreach($config['all']['stylesheets'] as $stylesheet)
      {
        $stylesheet = preg_replace("/\?.*/i", "", $stylesheet);
        if(file_exists(sfConfig::get('sf_web_dir') . '/css/' . $stylesheet))
        {
          $min_content .= file_get_contents(sfConfig::get('sf_web_dir') . '/css/' . $stylesheet);
        }
      }

      $filename = sha1($min_content) . '.css';
      if(file_exists(sfConfig::get('sf_web_dir') . '/css/min/' . $filename) || file_put_contents(sfConfig::get('sf_web_dir') . '/css/min/' . $filename, $min_content))
      {
        $config['all']['stylesheets'] = array('min/' . $filename);
      }
      foreach(glob(sfConfig::get('sf_web_dir') . '/css/min/*.css') as $stylesheet)
      {
        if(basename($stylesheet) != $filename)
        {
          @unlink($stylesheet);
        }
      }
      
      if(!is_dir(sfConfig::get('sf_web_dir') . '/js/min'))
      {
        mkdir(sfConfig::get('sf_web_dir') . '/js/min');
      }
      
      $min_content = '';
      foreach($config['all']['javascripts'] as $javascript)
      {
        $javascript = preg_replace("/\?.*/i", "", $javascript);
        if(file_exists(sfConfig::get('sf_web_dir') . '/js/' . $javascript))
        {
          $min_content .= file_get_contents(sfConfig::get('sf_web_dir') . '/js/' . $javascript);
        }
      }
      $filename = sha1($min_content) . '.js';
      if(file_exists(sfConfig::get('sf_web_dir') . '/css/min/' . $filename) || file_put_contents(sfConfig::get('sf_web_dir') . '/js/min/' . $filename, $min_content))
      {
        $config['all']['javascripts'] = array('min/' . $filename);
      }
      foreach(glob(sfConfig::get('sf_web_dir') . '/js/min/*.js') as $javascript)
      {
        if(basename($javascript) != $filename)
        {
          @unlink($javascript);
        }
      }
