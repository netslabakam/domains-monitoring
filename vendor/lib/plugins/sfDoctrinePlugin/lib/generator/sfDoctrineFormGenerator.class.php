...
class sfDoctrineFormGenerator extends sfGenerator
{
  ...
  {
    $options = array();
    $props = array('"autocomplete" => "off"');

    if ($column->isForeignKey())
    {
      $options[] = sprintf('\'model\' => $this->getRelatedModelName(\'%s\'), \'add_empty\' => %s', $column->getRelationKey('alias'), $column->isNotNull() ? 'false' : 'true');
    }
    else if (in_array($column->getDoctrineType(), array('set', 'enum')) && is_subclass_of($this->getWidgetClassForColumn($column), 'sfWidgetFormChoiceBase'))
    {
      $options[] = '\'choices\' => '.$this->arrayExport(key($column['values']) === 0 ? array_combine($column['values'], $column['values']) : $column['values']);
      if($column->getDoctrineType() === 'set'){
          $options[] = '\'multiple\' => true';
          $options[] = '\'expanded\' => true';
      }
    }
    if ($this->getWidgetClassForColumn($column) == 'sfWidgetFormInputText')
    {
      $props[] = '"size" => ' . ($column->getLength() ? ($column->getLength() > 65 ? round($column->getLength() / 4) : $column->getLength()) : '65');
      if($column->getLength() )
      {
        $props[] = '"maxlength" => ' . $column->getLength() . '';
      }
    }
    if ($this->getWidgetClassForColumn($column) == 'sfWidgetFormTextarea')
    {
      $props[] = '"rows" => 5';
      $props[] = '"cols" => 70';
    }
    ...
    if($column->isNotNull() && $this->getWidgetClassForColumn($column) !== 'sfWidgetFormInputCheckbox')
    {
      $props[] = '"required" => true';
    }
    return sprintf('array(%s), array(%s)', implode(', ', $options), implode(', ', $props));
  }
    ...
    else
    {
      switch ($column->getDoctrineType())
      {
        case 'string':
          ...
          break;
        case 'enum':
          $options[] = '\'choices\' => '.$this->arrayExport(key($column['values']) === 0 ? $column['values'] : array_keys($column['values']));
          break;
        case 'set':
          $options[] = '\'choices\' => '.$this->arrayExport(key($column['values']) === 0 ? $column['values'] : array_keys($column['values']));
          $options[] = '\'multiple\' => true';
