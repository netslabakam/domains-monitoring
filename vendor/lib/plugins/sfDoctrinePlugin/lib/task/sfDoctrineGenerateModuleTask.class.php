...
EOF
    ...
    $def_columns = array(
      'order_field' => 'Порядок сортировки',
      'is_activated' => 'Опубликован',
      'title' => 'Название',
    );
    $columns = Doctrine_Core::getTable($arguments['model'])->getColumnNames();
    $fields = array();
    
    foreach($columns as $column)
    {
      $fields[] = '        ' . $column . ': {label: ' . (array_key_exists($column, $def_columns) ? $def_columns[$column] : "''") . '}';
    }
    $this->constants['FIELDS'] = implode("\n", $fields);
    $this->getFilesystem()->replaceTokens($finder->in($moduleDir), '##', '##', $this->constants);
  }
}
