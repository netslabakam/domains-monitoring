...
class sfWidgetFormInputCheckbox extends sfWidgetFormInput
{
  ...
  protected function configure($options = array(), $attributes = array())
  {
    ...
    $this->setOption('with_label', false);
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    if(count($attributes) > 0){
        foreach ($attributes as $key => $attribute){
            $attributes[$key] = $key . '="' . $attribute . '"';
        }
    }

    $id = parent::generateId($name);
    return '<input id="' . $id . '" name="' . $name . '" type="checkbox" ' . implode(" ", $attributes) . ' value="1" />';
  }
}
