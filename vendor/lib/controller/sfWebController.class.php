...
abstract class sfWebController extends sfController
{
  ...
  public function genUrl($parameters = array(), $absolute = false)
  {
    ...
    if($route == '' && count($parameters) == 3 && array_key_exists('module', $parameters) && array_key_exists('action', $parameters) && array_key_exists('id', $parameters))
    {
      return (isset($_SERVER['PATH_PREFIX']) ? $_SERVER['PATH_PREFIX'] : '') . '/' . $parameters['module'] . '/' . $parameters['id'] . '/' . $parameters['action'];
    }
