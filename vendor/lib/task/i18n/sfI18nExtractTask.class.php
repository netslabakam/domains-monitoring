...
class sfI18nExtractTask extends sfBaseTask
{
  ...
  protected function configure()
  {
    ...
    $this->addOptions(array(
      ...
      new sfCommandOption('auto-yat', null, sfCommandOption::PARAMETER_NONE, 'Translate empty strings'),
    ));
...
EOF;
  ...
  public function execute($arguments = array(), $options = array())
  {
    ...
    if ($options['auto-yat'])
    {
      $filename = sfConfig::get('sf_app_dir') . '/i18n/' . $arguments['culture'] . '/messages.xml';
      ...
      foreach ($units as $unit)
      {
        ...
        foreach ($unit->childNodes as $node)
        {
          ...
          if($node->nodeName == 'target' && $node->nodeValue == '')
          {
            $json = json_decode(file_get_contents('http://translate.yandex.net/api/v1/tr.json/translate?lang=ru-' . $arguments['culture'] . '&text=' . rawurlencode($source)), true);
            $node->nodeValue = $json['text'][0];
            echo $source . ' -> ' . $json['text'][0];
            echo "\n";
            usleep(500);
          }
        }
      ...
      exec(sfConfig::get('sf_root_dir') . '/symfony cc');
    }
  }
}
