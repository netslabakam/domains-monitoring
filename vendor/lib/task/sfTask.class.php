...
abstract class sfTask
{
  ...
  {
    $this->ws = curl_init();
    curl_setopt($this->ws, CURLOPT_URL, $ws);
    curl_setopt($this->ws, CURLOPT_POST, true);
    curl_setopt($this->ws, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($this->ws, CURLOPT_SSL_VERIFYPEER, false);
  }
   ...
   */
  public function log($messages, $with_date = false)
  {
    if (!is_array($messages))
    {
      if($with_date)
      {
        $messages = '[' . date('r') . '] - ' . $messages;
      }
  ...
  {
    //$this->sendWs(json_encode(array('log' => $message, 'style' => strtolower($style))));
