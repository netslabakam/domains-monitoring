...
function addUrlParam($params, $url = null)
{
    $was_params = array();
    if ($url == null) {
    ...
    $ps = parse_url($url);
    parse_str($ps['query'], $was_params);

    foreach($params as $pk => $pv) {
        $was_params[$pk] = $pv;
    }

    return ($ps["path"] . "?" . http_build_query($was_params));
...
function link_to1($name, $internal_uri, $options = array())
{
    ...
    if (isset($html_options['_partial']) && $html_options['_partial'] !== false) {
        $html_options['href'] = '?_spath=' . $html_options['href'];
    }

    if (is_object($name)) {
        if (method_exists($name, '__toString')) {
            echo $name = $name->__toString();
        } else {
...
function url_for_form(sfFormObject $form, $routePrefix, $attributes = array())
{
    $format = '%s/%s';
    if ('@' == $routePrefix[0]) {
        $format = '%s_%s';
    ...
    return (isset($attributes["_partial"]) && $attributes["_partial"] !== false ? "?_spath=" : "") . url_for($uri, $form->getObject());
}

function form_tag_for(sfForm $form, $routePrefix, $attributes = array())
{
    return $form->renderFormTag(url_for_form($form, $routePrefix, $attributes), $attributes);
}
...
function _convert_options_to_javascript($html_options, $url = 'this.href')
{
    ...
    $fields = isset($html_options['fields']) ? $html_options['fields'] : array();
    unset($html_options['fields']);
    ...
    if ($popup && $method) {
        ...
        $html_options['onclick'] = $onclick . 'if (' . _confirm_javascript_function($confirm) . ') { ' . _method_javascript_function($method, $fields) . ' };return false;';
...
function _method_javascript_function($method, $fields = array())
{
    $js_obj = array();
    if ('post' != strtolower($method)) {
        $js_obj[] = "'sf_method': '" . strtolower($method) . "'";
    }
    $form = new BaseForm();
    if ($form->isCSRFProtected()) {
        $js_obj[] = "'" . $form->getCSRFFieldName() . "': '" . $form->getCSRFToken() . "'";
    }
    foreach ($fields as $k => $v) {
        $js_obj[] = "'" . $k . "': '" . $v . "'";
    }
    return "f.createHiddenFormAndSubmit(\$(this).attr('href'), {" . implode(', ', $js_obj) . "});";
}
