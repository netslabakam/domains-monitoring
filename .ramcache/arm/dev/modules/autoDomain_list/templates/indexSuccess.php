...
<span class="lui__h1"><?php echo __('Домены', array(), 'messages') ?></span><sup
...
<?php if (!$sf_request->isXmlHttpRequest()): ?>
    ...
    <div class="batch_form__class">
        ...
        <table cellpadding="0" cellspacing="0" border="0">
            <tr>
                <?php include_partial('domain_list/list_actions', array('helper' => $helper, '_partial' => isset($_partial))) ?>
                <?php include_partial('domain_list/list_batch_actions', array('helper' => $helper, '_partial' => isset($_partial))) ?>
                <?php if (!isset($_partial)): ?>
                                    <td style="padding-right: 10px"><input style="background-color:#fff;width:360px;" type="text" autocomplete="off" value="<?php echo $sf_request->getParameter('q'); ?>"
                                                           class="type_search input_with_erase" id="domain_list_live_search" placeholder="Найти"/>
                    </td>
                                <?php endif; ?>
                                                <?php if ($pager->haveToPaginate()): ?>
                <td align="right" width="100%">
                    <div class="lui_pager">
                        <?php include_partial('domain_list/pagination', array('pager' => $pager, '_partial_filters' => (isset($_partial_filters) ? $_partial_filters : array()), '_partial' => isset($_partial))) ?>&nbsp;
    ...
    <form id="batch_form" action="<?php echo ($_partial ? '?_spath=' : '') . url_for('domain_list_collection', array('action' => 'batch')) ?>" method="post">


        <div id="lui_scroller" class="<?php echo (isset($_partial_no_scroll) ? '' : 'lui_scroller'); ?>">
            <div class="lui__scroller_wrapper"<?php echo (isset($_partial) ? ' style="padding:0 !important"' : ''); ?>><?php include_partial('domain_list/list', array('pager' => $pager, 'sort' => $sort, 'helper' => $helper, '_partial_filters' => (isset($_partial_filters) ? $_partial_filters->getRawValue() : array()), '_partial' => isset($_partial), '_partial_no_scroll' => isset($_partial_no_scroll))) ?></div>
        </div>
    ...
    <?php if (!$sf_request->isXmlHttpRequest()): ?>
        ...
        <script type="text/javascript">
            ...
            if ($('#domain_list_live_search').length) {


                var jqxhr_old = $('#domain_list_live_search').val().trim();
                var jqxhr_last_result = $('#domain_list_live_search').val().trim();
                ...
                var is_not_good_style_of_code = function (url) {
                    ...
                    jqxhr = $.ajax(url).done(function (html) {
                        ...
                        jqxhr_last_result = $('#domain_list_live_search').val().trim();
                ...
                $(document).ready(function () {
                    $('#domain_list_live_search').keyup(function (event) {
