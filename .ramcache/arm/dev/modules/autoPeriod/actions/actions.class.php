...
require_once(dirname(__FILE__).'/../lib/BasePeriodGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BasePeriodGeneratorHelper.class.php');
...
*/
abstract class autoPeriodActions extends sfActions
{
...
$user = $this->getUser();
$access = $this->getActionName() == 'batch' || $user->hasCredential('period_' . $this->getActionName());
if(!$access){
$access = $this->getActionName() == 'select_list' && ($user->hasCredential('period_edit') || $user->hasCredential('period_create'));
}
if (!$access){
...
public function executeNew(sfWebRequest $request)
{
    ...
    public function executeDelete(sfWebRequest $request)
    {
        ...
        if(!$request->isXmlHttpRequest())
        {
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('period');
        ...
        public function executeBatch(sfWebRequest $request)
    {
        ...
        if (!$this->getUser()->hasCredential('period_' . $action))
        {
            $this->forward(sfConfig::get('sf_secure_module'), sfConfig::get('sf_secure_action'));
        }

        $validator = new sfValidatorDoctrineChoice(array('multiple' => true, 'model' => 'Period'));
        try
        {
    ...
    protected function executeBatchMerge(sfWebRequest $request)
    {
        ...
        $primary = Doctrine::getTable(Period)->find($request->getParameter('batch_primary_id'));
...
protected function processForm(sfWebRequest $request, sfForm $form)
{
        ...
        if ($form->isValid())
    {
        ...
        if ($request->hasParameter('_save_and_add'))
        {
            ...
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('period_new');
            $this->redirect($redirect_url);
        }
        ...
        else
        {
            ...
            {
                $this->getUser()->setFlash('notice', $notice);
                $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->getController()->genUrl(array('sf_route' => 'period_edit', 'sf_subject' => $period));
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'period.page', $page, 'admin_module');
    }

    protected function getPage()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'period.page', 1, 'admin_module');
    }

    protected function buildQuery()
    {
        $tableMethod = $this->configuration->getTableMethod();
        $query = Doctrine::getTable('Period')->createQuery('r');
        if ($tableMethod)
        {
            $query = Doctrine::getTable('Period')->$tableMethod($query);
        }
    ...
    {
        if (null !== $sort = $this->getUser()->getAttribute($this->getPartialPrefix() . 'period.sort', null, 'admin_module'))
        {
            return $sort;
        }
        $this->setSort($this->configuration->getDefaultSort());
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'period.sort', null, 'admin_module');
    }
    ...
    protected function setSort(array $sort)
    {
        ...
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'period.sort', $sort, 'admin_module');
    }
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'period.search', $search, 'admin_module');
    }

    protected function setChecked($checked)
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'period.checked', $checked, 'admin_module');
    }

    protected function getSearch()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'period.search', false, 'admin_module');
    }

    protected function getChecked()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'period.checked', false, 'admin_module');
    }

    protected function isValidSortColumn($column)
    {
        return Doctrine::getTable('Period')->hasColumn($column);
    }
