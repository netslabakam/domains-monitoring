...
require_once(dirname(__FILE__).'/../lib/BaseServer_listGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseServer_listGeneratorHelper.class.php');
...
*/
abstract class autoServer_listActions extends sfActions
{
public function preExecute()
{
$this->configuration = new server_listGeneratorConfiguration();

$user = $this->getUser();
$access = $this->getActionName() == 'batch' || $user->hasCredential('server_list_' . $this->getActionName());
if(!$access){
$access = $this->getActionName() == 'select_list' && ($user->hasCredential('server_list_edit') || $user->hasCredential('server_list_create'));
}
if (!$access){
...
$this->helper = new server_listGeneratorHelper();
}
...
public function executeNew(sfWebRequest $request)
{
    ...
    $this->server_list = $this->form->getObject();
    }
    ...
    {
        $this->server_list = $this->getRoute()->getObject();
        $user = $this->getUser();
        $this->form = $this->configuration->getForm($this->server_list);
        $this->setPartialFilter();
    }
    ...
    public function executeDelete(sfWebRequest $request)
    {
        ...
        if(!$request->isXmlHttpRequest())
        {
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('server_list');
        ...
        public function executeBatch(sfWebRequest $request)
    {
        ...
        if (!$this->getUser()->hasCredential('server_list_' . $action))
        {
            $this->forward(sfConfig::get('sf_secure_module'), sfConfig::get('sf_secure_action'));
        }

        $validator = new sfValidatorDoctrineChoice(array('multiple' => true, 'model' => 'ServerList'));
        try
        {
    ...
    protected function executeBatchMerge(sfWebRequest $request)
    {
        ...
        $primary = Doctrine::getTable(ServerList)->find($request->getParameter('batch_primary_id'));
...
protected function processForm(sfWebRequest $request, sfForm $form)
{
        ...
        if ($form->isValid())
    {
        ...
        $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $server_list)));
        if ($request->hasParameter('_save_and_add'))
        {
            $this->getUser()->setFlash('notice', $notice.' You can add another one below.');
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('server_list_new');
            $this->redirect($redirect_url);
        }
        ...
        else
        {
            ...
            {
                $this->getUser()->setFlash('notice', $notice);
                $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->getController()->genUrl(array('sf_route' => 'server_list_edit', 'sf_subject' => $server_list));
    ...
    {
        $pager = $this->configuration->getPager('ServerList');
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'server_list.page', $page, 'admin_module');
    }

    protected function getPage()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'server_list.page', 1, 'admin_module');
    }

    protected function buildQuery()
    {
        $tableMethod = $this->configuration->getTableMethod();
        $query = Doctrine::getTable('ServerList')->createQuery('r');
        if ($tableMethod)
        {
            $query = Doctrine::getTable('ServerList')->$tableMethod($query);
        }
    ...
    {
        if (null !== $sort = $this->getUser()->getAttribute($this->getPartialPrefix() . 'server_list.sort', null, 'admin_module'))
        {
            return $sort;
        }
        $this->setSort($this->configuration->getDefaultSort());
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'server_list.sort', null, 'admin_module');
    }
    ...
    protected function setSort(array $sort)
    {
        ...
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'server_list.sort', $sort, 'admin_module');
    }
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'server_list.search', $search, 'admin_module');
    }

    protected function setChecked($checked)
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'server_list.checked', $checked, 'admin_module');
    }

    protected function getSearch()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'server_list.search', false, 'admin_module');
    }

    protected function getChecked()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'server_list.checked', false, 'admin_module');
    }

    protected function isValidSortColumn($column)
    {
        return Doctrine::getTable('ServerList')->hasColumn($column);
    }
