...
<?php
$fa = $sf_user->getAttribute('hosting_plans.filters', $configuration, 'admin_module');
$fac = array();
foreach($fa as $f)
{
if(!is_null($f))
...
<div class="sf_admin_filter__wrapper">
    ...
    <div class="sf_admin_filter">
        ...
        <div id="sf_admin_filter_form">
            <form action="<?php echo url_for('hosting_plans_collection', array('action' => 'filter')) ?>" method="post">
                <?php foreach ($configuration->getFormFilterFields($form) as $name => $field): ?>
                <?php if ((isset($form[$name]) && $form[$name]->isHidden()) || (!isset($form[$name]) && $field->isReal())) continue ?>
                <?php include_partial('hosting_plans/filters_field', array(
                'name'       => $name,
                ...
                <input type="submit" class="pseudo_button no_ajax" value="Применить" />&nbsp;&nbsp;<?php echo link_to(__('Reset', array(), 'sf_admin'), 'hosting_plans_collection', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post', 'class' => 'lui_pseudo')) ?>
