...
require_once(dirname(__FILE__).'/../lib/BaseClient_listGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseClient_listGeneratorHelper.class.php');
...
*/
abstract class autoClient_listActions extends sfActions
{
public function preExecute()
{
$this->configuration = new client_listGeneratorConfiguration();

$user = $this->getUser();
$access = $this->getActionName() == 'batch' || $user->hasCredential('client_list_' . $this->getActionName());
if(!$access){
$access = $this->getActionName() == 'select_list' && ($user->hasCredential('client_list_edit') || $user->hasCredential('client_list_create'));
}
if (!$access){
...
$this->helper = new client_listGeneratorHelper();
}
...
public function executeNew(sfWebRequest $request)
{
    ...
    $this->client_list = $this->form->getObject();
    }
    ...
    {
        $this->client_list = $this->getRoute()->getObject();
        $user = $this->getUser();
        $this->form = $this->configuration->getForm($this->client_list);
        $this->setPartialFilter();
    }
    ...
    public function executeDelete(sfWebRequest $request)
    {
        ...
        if(!$request->isXmlHttpRequest())
        {
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('client_list');
        ...
        public function executeBatch(sfWebRequest $request)
    {
        ...
        if (!$this->getUser()->hasCredential('client_list_' . $action))
        {
            $this->forward(sfConfig::get('sf_secure_module'), sfConfig::get('sf_secure_action'));
        }

        $validator = new sfValidatorDoctrineChoice(array('multiple' => true, 'model' => 'ClientList'));
        try
        {
    ...
    protected function executeBatchMerge(sfWebRequest $request)
    {
        ...
        $primary = Doctrine::getTable(ClientList)->find($request->getParameter('batch_primary_id'));
...
protected function processForm(sfWebRequest $request, sfForm $form)
{
        ...
        if ($form->isValid())
    {
        ...
        $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $client_list)));
        if ($request->hasParameter('_save_and_add'))
        {
            $this->getUser()->setFlash('notice', $notice.' You can add another one below.');
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('client_list_new');
            $this->redirect($redirect_url);
        }
        ...
        else
        {
            ...
            {
                $this->getUser()->setFlash('notice', $notice);
                $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->getController()->genUrl(array('sf_route' => 'client_list_edit', 'sf_subject' => $client_list));
    ...
    {
        $pager = $this->configuration->getPager('ClientList');
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'client_list.page', $page, 'admin_module');
    }

    protected function getPage()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'client_list.page', 1, 'admin_module');
    }

    protected function buildQuery()
    {
        $tableMethod = $this->configuration->getTableMethod();
        $query = Doctrine::getTable('ClientList')->createQuery('r');
        if ($tableMethod)
        {
            $query = Doctrine::getTable('ClientList')->$tableMethod($query);
        }
    ...
    {
        if (null !== $sort = $this->getUser()->getAttribute($this->getPartialPrefix() . 'client_list.sort', null, 'admin_module'))
        {
            return $sort;
        }
        $this->setSort($this->configuration->getDefaultSort());
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'client_list.sort', null, 'admin_module');
    }
    ...
    protected function setSort(array $sort)
    {
        ...
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'client_list.sort', $sort, 'admin_module');
    }
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'client_list.search', $search, 'admin_module');
    }

    protected function setChecked($checked)
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'client_list.checked', $checked, 'admin_module');
    }

    protected function getSearch()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'client_list.search', false, 'admin_module');
    }

    protected function getChecked()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'client_list.checked', false, 'admin_module');
    }

    protected function isValidSortColumn($column)
    {
        return Doctrine::getTable('ClientList')->hasColumn($column);
    }
