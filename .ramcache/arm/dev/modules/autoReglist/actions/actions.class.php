...
require_once(dirname(__FILE__).'/../lib/BaseReglistGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseReglistGeneratorHelper.class.php');
...
*/
abstract class autoReglistActions extends sfActions
{
public function preExecute()
{
$this->configuration = new reglistGeneratorConfiguration();

$user = $this->getUser();
$access = $this->getActionName() == 'batch' || $user->hasCredential('reglist_' . $this->getActionName());
if(!$access){
$access = $this->getActionName() == 'select_list' && ($user->hasCredential('reglist_edit') || $user->hasCredential('reglist_create'));
}
if (!$access){
...
$this->helper = new reglistGeneratorHelper();
}
...
public function executeNew(sfWebRequest $request)
{
    ...
    $this->reglist = $this->form->getObject();
    }
    ...
    {
        $this->reglist = $this->getRoute()->getObject();
        $user = $this->getUser();
        $this->form = $this->configuration->getForm($this->reglist);
        $this->setPartialFilter();
    }
    ...
    public function executeDelete(sfWebRequest $request)
    {
        ...
        if(!$request->isXmlHttpRequest())
        {
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('reglist');
        ...
        public function executeBatch(sfWebRequest $request)
    {
        ...
        if (!$this->getUser()->hasCredential('reglist_' . $action))
        {
            $this->forward(sfConfig::get('sf_secure_module'), sfConfig::get('sf_secure_action'));
        }

        $validator = new sfValidatorDoctrineChoice(array('multiple' => true, 'model' => 'reglist'));
        try
        {
    ...
    protected function executeBatchMerge(sfWebRequest $request)
    {
        ...
        $primary = Doctrine::getTable(reglist)->find($request->getParameter('batch_primary_id'));
...
protected function processForm(sfWebRequest $request, sfForm $form)
{
        ...
        if ($form->isValid())
    {
        ...
        $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $reglist)));
        if ($request->hasParameter('_save_and_add'))
        {
            $this->getUser()->setFlash('notice', $notice.' You can add another one below.');
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('reglist_new');
            $this->redirect($redirect_url);
        }
        ...
        else
        {
            ...
            {
                $this->getUser()->setFlash('notice', $notice);
                $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->getController()->genUrl(array('sf_route' => 'reglist_edit', 'sf_subject' => $reglist));
    ...
    {
        $pager = $this->configuration->getPager('reglist');
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'reglist.page', $page, 'admin_module');
    }

    protected function getPage()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'reglist.page', 1, 'admin_module');
    }

    protected function buildQuery()
    {
        $tableMethod = $this->configuration->getTableMethod();
        $query = Doctrine::getTable('reglist')->createQuery('r');
        if ($tableMethod)
        {
            $query = Doctrine::getTable('reglist')->$tableMethod($query);
        }
    ...
    {
        if (null !== $sort = $this->getUser()->getAttribute($this->getPartialPrefix() . 'reglist.sort', null, 'admin_module'))
        {
            return $sort;
        }
        $this->setSort($this->configuration->getDefaultSort());
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'reglist.sort', null, 'admin_module');
    }
    ...
    protected function setSort(array $sort)
    {
        ...
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'reglist.sort', $sort, 'admin_module');
    }
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'reglist.search', $search, 'admin_module');
    }

    protected function setChecked($checked)
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'reglist.checked', $checked, 'admin_module');
    }

    protected function getSearch()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'reglist.search', false, 'admin_module');
    }

    protected function getChecked()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'reglist.checked', false, 'admin_module');
    }

    protected function isValidSortColumn($column)
    {
        return Doctrine::getTable('reglist')->hasColumn($column);
    }
