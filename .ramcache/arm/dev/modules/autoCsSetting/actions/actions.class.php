...
require_once(dirname(__FILE__).'/../lib/BaseCsSettingGeneratorConfiguration.class.php');
require_once(dirname(__FILE__).'/../lib/BaseCsSettingGeneratorHelper.class.php');
...
*/
abstract class autoCsSettingActions extends sfActions
{
public function preExecute()
{
$this->configuration = new csSettingGeneratorConfiguration();

$user = $this->getUser();
$access = $this->getActionName() == 'batch' || $user->hasCredential('csSetting_' . $this->getActionName());
if(!$access){
$access = $this->getActionName() == 'select_list' && ($user->hasCredential('csSetting_edit') || $user->hasCredential('csSetting_create'));
}
if (!$access){
...
$this->helper = new csSettingGeneratorHelper();
}
...
public function executeNew(sfWebRequest $request)
{
    ...
    $this->cs_setting = $this->form->getObject();
    }
    ...
    {
        $this->cs_setting = $this->getRoute()->getObject();
        $user = $this->getUser();
        $this->form = $this->configuration->getForm($this->cs_setting);
        $this->setPartialFilter();
    }
    ...
    public function executeDelete(sfWebRequest $request)
    {
        ...
        if(!$request->isXmlHttpRequest())
        {
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('cs_setting');
...
protected function processForm(sfWebRequest $request, sfForm $form)
{
        ...
        if ($form->isValid())
    {
        ...
        $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $cs_setting)));
        if ($request->hasParameter('_save_and_add'))
        {
            $this->getUser()->setFlash('notice', $notice.' You can add another one below.');
            $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->generateUrl('cs_setting_new');
            $this->redirect($redirect_url);
        }
        ...
        else
        {
            ...
            {
                $this->getUser()->setFlash('notice', $notice);
                $redirect_url = (isset($this->_partial) ? $_SERVER['DOCUMENT_URI'] . '?_spath=' : '') . $this->getController()->genUrl(array('sf_route' => 'cs_setting_edit', 'sf_subject' => $cs_setting));
    ...
    {
        $pager = $this->configuration->getPager('csSetting');
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'csSetting.page', $page, 'admin_module');
    }

    protected function getPage()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'csSetting.page', 1, 'admin_module');
    }

    protected function buildQuery()
    {
        $tableMethod = $this->configuration->getTableMethod();
        $query = Doctrine::getTable('csSetting')->createQuery('r');
        if ($tableMethod)
        {
            $query = Doctrine::getTable('csSetting')->$tableMethod($query);
        }
    ...
    {
        if (null !== $sort = $this->getUser()->getAttribute($this->getPartialPrefix() . 'csSetting.sort', null, 'admin_module'))
        {
            return $sort;
        }
        $this->setSort($this->configuration->getDefaultSort());
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'csSetting.sort', null, 'admin_module');
    }
    ...
    protected function setSort(array $sort)
    {
        ...
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'csSetting.sort', $sort, 'admin_module');
    }
    ...
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'csSetting.search', $search, 'admin_module');
    }

    protected function setChecked($checked)
    {
        $this->getUser()->setAttribute($this->getPartialPrefix() . 'csSetting.checked', $checked, 'admin_module');
    }

    protected function getSearch()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'csSetting.search', false, 'admin_module');
    }

    protected function getChecked()
    {
        return $this->getUser()->getAttribute($this->getPartialPrefix() . 'csSetting.checked', false, 'admin_module');
    }

    protected function isValidSortColumn($column)
    {
        return Doctrine::getTable('csSetting')->hasColumn($column);
    }
